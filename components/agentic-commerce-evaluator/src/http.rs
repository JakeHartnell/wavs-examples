/// Synchronous HTTP GET and POST using raw WASI bindings.
///
/// Avoids wstd (which imports wasi:random@0.2.9, incompatible with the WAVS
/// operator world that provides wasi:http@0.2.0 only).
...
// ─────────────────────────────────────────────────────────────────
// POST (used for JSON-RPC eth_call)
// ─────────────────────────────────────────────────────────────────

pub fn post(url: &str, body_bytes: &[u8]) -> Result<Vec<u8>, String> {
    let (scheme, authority, path_query) = parse_url(url)?;
    ...
    // Write body
    {
        ...
        stream.blocking_write_and_flush(body_bytes).map_err(|e| format!("write body: {:?}", e))?;
        drop(stream);

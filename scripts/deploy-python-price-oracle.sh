#!/usr/bin/env bash
# =============================================================================
# deploy-python-price-oracle.sh — Full end-to-end deploy for python-evm-price-oracle
#
# What it does:
#   1. Uses pre-compiled Python WASM from compiled/python_evm_price_oracle.wasm
...
#   - The Python WASM embeds CPython (~41MB). It's larger than Go/Rust/JS but
#     opens the door to Python's massive ecosystem (numpy, transformers, etc.)
#   - Pass a CMC ID as the trigger input (1=BTC, 1027=ETH, 5805=AVAX, etc.)
#   - Build the component first: make -C components/python-evm-price-oracle wasi-build
#
# Usage:
#   ./scripts/deploy-python-price-oracle.sh
#   CMC_ID=1027 ./scripts/deploy-python-price-oracle.sh
# =============================================================================
set -euo pipefail
...
# =============================================================================
# 1. Check pre-compiled Python WASM
# =============================================================================
PYTHON_WASM="compiled/python_evm_price_oracle.wasm"
[ -f "$PYTHON_WASM" ] || die "Python WASM not found at $PYTHON_WASM — run: make -C components/python-evm-price-oracle wasi-build"
info "Using pre-compiled Python WASM: $PYTHON_WASM ($(du -sh "$PYTHON_WASM" | cut -f1))"

# =============================================================================
# 2. Check aggregator WASM
# =============================================================================
info "Checking aggregator component..."
AGG_WASM="compiled/aggregator.wasm"
[ -f "$AGG_WASM" ] || die "aggregator.wasm not found at $AGG_WASM — run: make build (from repo root)"
...
# =============================================================================
info "Uploading Python price oracle component (this may take a moment — 41MB)..."
PYTHON_DIGEST=$(curl -sf -X POST "$WAVS_URL/dev/components" \
  -H "Content-Type: application/wasm" \
  --data-binary @"$PYTHON_WASM" | python3 -c "import json,sys; print(json.load(sys.stdin)['digest'])")
success "Python price oracle digest: $PYTHON_DIGEST"
...
export SM_ADDR TRIGGER_ADDR SUBMIT_ADDR EVENT_HASH PYTHON_DIGEST AGG_DIGEST CHAIN_ID

python3 - <<PYEOF
import json, os
svc = {
  "name": "python-evm-price-oracle",
  "status": "active",
  ...
  "workflows": {
    "default": {
      ...
      "component": {
        "source": {"digest": os.environ["PYTHON_DIGEST"]},
...
with open("/tmp/python-price-oracle-svc.json", "w") as f:
    json.dump(svc, f)
PYEOF

SERVICE_HASH=$(curl -sf -X POST "$WAVS_URL/dev/services" \
  -H "Content-Type: application/json" \
  -d @/tmp/python-price-oracle-svc.json | python3 -c "import json,sys; print(json.load(sys.stdin)['hash'])")
...
info "Waiting 60 seconds for WAVS to process (Python component is ~41MB — CPython embedded)..."
sleep 60
...
"
  ...
  echo ""
  echo "════════════════════════════════════════════"
  success "PYTHON PRICE ORACLE END-TO-END COMPLETE! 🐍"

#!/usr/bin/env bash
# =============================================================================
# deploy-js-price-oracle.sh — Full end-to-end deploy for js-evm-price-oracle
#
# What it does:
#   1. Uses pre-compiled JS WASM from compiled/js_evm_price_oracle.wasm
...
#   - The JS WASM embeds SpiderMonkey (~15MB). It's larger than Go/Rust but
#     provides a full JS runtime with fetch(), async/await, etc.
#   - Known issue (componentize-js 0.19.3): Never let exceptions escape `run`.
#     The component wraps all logic in try/catch and returns Err variants safely.
#   - Pass a CMC ID as the trigger input (1=BTC, 1027=ETH, 5805=AVAX, etc.)
#
# Usage:
#   ./scripts/deploy-js-price-oracle.sh
#   CMC_ID=1027 ./scripts/deploy-js-price-oracle.sh
# =============================================================================
set -euo pipefail
...
# =============================================================================
# 1. Check pre-compiled JS WASM
# =============================================================================
JS_WASM="compiled/js_evm_price_oracle.wasm"
[ -f "$JS_WASM" ] || die "JS WASM not found at $JS_WASM — run: make -C components/js-evm-price-oracle wasi-build"
info "Using pre-compiled JS WASM: $JS_WASM ($(du -sh "$JS_WASM" | cut -f1))"
...
# =============================================================================
info "Uploading JS price oracle component..."
JS_DIGEST=$(curl -sf -X POST "$WAVS_URL/dev/components" \
  -H "Content-Type: application/wasm" \
  --data-binary @"$JS_WASM" | python3 -c "import json,sys; print(json.load(sys.stdin)['digest'])")
success "JS price oracle digest: $JS_DIGEST"
...
"
  ...
  echo ""
  echo "════════════════════════════════════════"
  success "JS PRICE ORACLE END-TO-END COMPLETE!"
